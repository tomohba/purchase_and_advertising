...
- (void)viewDidLoad {
	...
	// アプリ内課金プロダクト情報を取得する
	myProduct = nil;
	myProductRequest = [[SKProductsRequest alloc] initWithProductIdentifiers:[NSSet setWithObject:@"AutoRenewableProduct"]];
...
- (IBAction)helloButtonOnTouch:(id)sender {
	// すてにプロダクトを購入済みか判定する
	NSTimeInterval expiresDate = [[[NSUserDefaults standardUserDefaults] objectForKey:@"expires_date"] doubleValue] / 1000.0;
